...
do
    ...
    if [ "$4" == "$number" ]; then
	printf "`diff $TESTD/$out $dirn/result.$number -y`\n"
	printf "Your result is located at: $dirn/result.$number\nIt was compared to: $TESTD/$out\nCalled with: $comm $args\nStdin: $TESTD/$inp\n"
    fi
    let number=number+1
    if ! [[ "$4" == "" || "$4" == "$number" ]]; then
	continue
